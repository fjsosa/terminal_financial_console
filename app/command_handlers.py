...
class RuntimeConfigCommands:
    ...
    def add_symbol(self, tokens: list[str]) -> None:
        app = self.host
        if len(tokens) < 4:
            app._log("[yellow]Usage:[/] :add <symbol> <crypto|stock> <group> [name]")
            return
        ...
        if symbol_type not in {"crypto", "stock"}:
            app._log("[yellow]Add failed:[/] type must be crypto or stock")
            return
        if not symbol or not group_name:
            app._log("[yellow]Add failed:[/] symbol and group are required")
            return
        if app._find_symbol_entry(symbol):
            app._log(f"[yellow]Add failed:[/] symbol already exists: {symbol}")
            return
        group_idx = app._find_group_index(group_name)
        if group_idx is None:
            app._log(f"[yellow]Add failed:[/] group not found: {group_name}")
            return
        ...
        if not isinstance(symbols, list):
            app._log(f"[yellow]Add failed:[/] invalid group schema: {group_name}")
            return
        symbols.append(item)
        app._apply_market_groups_change(resolve_missing_names=not bool(name))
        if not app._persist_config():
            app._log("[yellow]Add warning:[/] could not persist config.yml")
            return
        app._log(f"[#2ec4b6]CONFIG[/] added {symbol} ({symbol_type}) to group '{group_name}'")

    def delete_symbol(self, tokens: list[str]) -> None:
        app = self.host
        if len(tokens) != 2:
            app._log("[yellow]Usage:[/] :del <symbol>")
            return
        symbol = tokens[1].strip().upper()
        found = app._find_symbol_entry(symbol)
        if not found:
            app._log(f"[yellow]Delete failed:[/] symbol not found: {symbol}")
            return
        ...
        if not app._persist_config():
            app._log("[yellow]Delete warning:[/] could not persist config.yml")
            return
        app._log(f"[#2ec4b6]CONFIG[/] deleted {symbol} from group '{group_name}'")

    def move_symbol(self, tokens: list[str]) -> None:
        app = self.host
        if len(tokens) < 3:
            app._log("[yellow]Usage:[/] :mv <symbol> <group>")
            return
        symbol = tokens[1].strip().upper()
        destination_name = " ".join(tokens[2:]).strip()
        found = app._find_symbol_entry(symbol)
        if not found:
            app._log(f"[yellow]Move failed:[/] symbol not found: {symbol}")
            return
        from_group_idx, item_idx, item = found
        to_group_idx = app._find_group_index(destination_name)
        if to_group_idx is None:
            app._log(f"[yellow]Move failed:[/] destination group not found: {destination_name}")
            return
        if to_group_idx == from_group_idx:
            app._log(f"[#6f8aa8]CONFIG[/] {symbol} already in group '{destination_name}'")
            return
        ...
        if not isinstance(symbols_src, list) or not isinstance(symbols_dst, list):
            app._log("[yellow]Move failed:[/] invalid group schema")
            return
        moved = dict(item)
        ...
        if not symbols_src:
            ...
            app._log(f"[#6f8aa8]CONFIG[/] removed empty group '{source_name}'")
        app._apply_market_groups_change(resolve_missing_names=False)
        if not app._persist_config():
            app._log("[yellow]Move warning:[/] could not persist config.yml")
            return
        app._log(f"[#2ec4b6]CONFIG[/] moved {symbol} from '{source_name}' to '{destination_name}'")
    ...
    def edit_symbol(self, tokens: list[str]) -> None:
        ...
        found = app._find_symbol_entry(symbol)
        if not found:
            app._log(f"[yellow]Edit failed:[/] symbol not found: {symbol}")
            return
        group_idx, item_idx, item = found
        updates: dict[str, str] = {}
        for part in tokens[2:]:
            if "=" not in part:
                app._log(f"[yellow]Edit failed:[/] invalid token '{part}', expected key=value")
                return
            key, value = part.split("=", 1)
            k = key.strip().lower()
            v = value.strip()
            if k not in {"group", "type", "name"}:
                app._log(f"[yellow]Edit failed:[/] unsupported field '{k}'")
                return
            updates[k] = v
        ...
        if destination_group:
            ...
            if dest_idx is None:
                app._log(f"[yellow]Edit failed:[/] group not found: {destination_group}")
                return
        new_type = updates.get("type")
        if new_type and app._normalize_symbol_type(symbol, new_type) not in {"crypto", "stock"}:
            app._log("[yellow]Edit failed:[/] type must be crypto or stock")
            return
        ...
        if destination_group:
            ...
            if dest_idx != group_idx:
                ...
                if not isinstance(src_symbols, list) or not isinstance(dst_symbols, list):
                    app._log("[yellow]Edit failed:[/] invalid group schema")
                    return
                moved = dict(item)
        ...
        if not app._persist_config():
            app._log("[yellow]Edit warning:[/] could not persist config.yml")
            return
        app._log(f"[#2ec4b6]CONFIG[/] updated {symbol}")
